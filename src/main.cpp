...
uint32_t period = 25, modules_period = 5000;
...
void setup(void)
{
	...
	while (1)
	{
		...
		float adc_value = analogRead(ROTARY_2);
		...
		else if (volatage <= 0.63){
 		   power_level = 1;
		}
		else if (volatage <= 0.8){
			power_level = 2;
		}
   		else if (volatage <= 1.3){
			power_level = 3;
		}
    	else if (volatage <= 2){
			power_level = 4;
		...
		//bson.append(BSON_LAPCOUNT, (int32_t)lap_count); 
#endif
		...
		if (Serial2.available() > 0)
		{
			...
			int8_t value1;
			Serial.read(buffer_calibration, 4);
			if (memcmp(buffer_calibration, "\xFF\xFF\xFF\xFF", 4) == 0)
			{
				...
				EncodingUnion packet;
				Serial.read(buffer_calibration, size_of_string);
